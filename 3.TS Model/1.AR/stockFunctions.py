...
def conversion(y_train,stk_data):
    ...
    for i in range(len(y_train)):
        ...
        #.iloc[i]: Accesses the row at the i-th position (not by label).y_train[i]: Takes the i-th value from another array or list y_train
        #this line copies the i-th value from y_train into the i-th row of Actual_y_train
...
def rmsemape(y_Test,predicted_stock_price_test_ori):
    from sklearn.metrics import mean_squared_error #learn formula MSE= 1/n ∑(Act-pre)²
    ...
    from sklearn.metrics import mean_absolute_percentage_error #learn formula MAPE = 1/n ∑((Act-pre)/Act) * 100
